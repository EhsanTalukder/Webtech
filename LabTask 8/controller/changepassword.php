...
if(isset($_POST['submit']) && isset($_SESSION['username']))
{
 ...
 {
    $_SESSION['npasswordE']=  "New password required";
   $flag=0;
 }
 else {
   ...
   {
     $_SESSION['npasswordE']= "Password must not be less than eight 8 characters";
     $flag=0;
   }
   else {
 ...
 if(empty($_POST["rnpassword"]))
 {
   ...
   {
      $_SESSION['rnpasswordE']="Retype new password must not be less than eight (8) characters";
     $flag=0;
   }
   else {
     if(substr_count($rnpassword,"@")<1 || substr_count($rnpassword,"#")<1 || substr_count($rnpassword,"%")<1 || substr_count($rnpassword,"$")<1)
     {
        $_SESSION['rnpasswordE']= "Retype Password must contain at least one of the special characters (@, #, $,%)";
       $flag=0;
     }
   }
 }
  ...
  else{header('Location: ../change password.php');}
}
